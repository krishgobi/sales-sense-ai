...
def create_worker():
    if request.method == 'POST':
        ...
        if workers.find_one({'email': email}):
...
def add_to_cart():
    ...
    try:
        ...
        if not product or variant_index >= len(product['variants']):
            return jsonify({'error': 'Invalid product or variant'}), 400
        ...
        if cart_key in session['cart']:
            ...
            session['cart'][cart_key] = {
                ...
                'variant_index': variant_index,  # Store the variant index
                'variant_quantity': variant['quantity'],
                'price': variant['price'],
                'quantity': quantity,
                'cart_key': cart_key  # Store the cart key for reference
            }
    ...
    try:
        ...
        for purchase in purchases:
            ...
            products_update.update_one(
                ...
                {'$inc': {f'variants.{cart[f"{purchase["product_id"]}_{purchase["variant_index"]}"]["variant_index"]}.stock': -purchase['quantity']}}
            )
