...
def admin_panel():
    ...
    stats = {
        ...
        'total_sales': sum((sale.get('total_price', 0) or sale.get('price', 0) * sale.get('quantity', 0)) 
                          for sale in products_sold.find()),
        'sales_today': sum((sale.get('total_price', 0) or sale.get('price', 0) * sale.get('quantity', 0)) 
                          for sale in products_sold.find({
    ...
    for user in users_data:
        ...
        user['total_spent'] = sum(order['total_price'] for order in user_orders)
    ...
    for sale in products_sold.find():
        ...
        if product:
            ...
            category_sales[category] += sale['total_price']
