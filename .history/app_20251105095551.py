...
def admin_dashboard():
    ...
    try:
        for sale in products_sold.find({'date': {'$gte': past_week}}):
            ...
            if date_str not in sales_by_date:
                sales_by_date[date_str] = 0.0  # Initialize as float
            # Calculate sale amount using helper function and ensure it's a float
            try:
                sale_amount = float(calculate_sale_amount(sale))
                sales_by_date[date_str] += sale_amount
            except (TypeError, ValueError) as e:
                print(f"Error converting sale amount for sale {sale.get('_id')}: {e}")
                continue
    except Exception as e:
        print(f"Error fetching sales data: {e}")
        sales_by_date = {}
    
    # Ensure all dates between past_week and today are represented
    date_range = []
    current = past_week
    while current <= today:
    ...
    # Create the sales data dictionary with sorted dates and ensure JSON serializable
    sorted_dates = sorted(date_range)
    sales_values = []
    for date in sorted_dates:
        value = sales_by_date.get(date, 0.0)
        # Ensure the value is a proper float, not a function or other object
        if callable(value):
    ...
    print(f"Sales data debug: {sales_data}")  # Debug print
        ...
        for sale in products_sold.find():
            ...
            if product:
                ...
                if category not in category_sales:
                    category_sales[category] = 0.0  # Initialize as float
                # Calculate sale amount using helper function
                try:
                    sale_amount = float(calculate_sale_amount(sale))  # Convert to float
                    category_sales[category] += sale_amount
                except (TypeError, ValueError) as e:
                    print(f"Error converting category sale amount for sale {sale.get('_id')}: {e}")
                    continue
    except Exception as e:
        print(f"Error fetching category data: {e}")
        category_sales = {}

    # Convert dictionary values to list of floats with safety checks
    category_values = []
    category_labels = []
    for label, value in category_sales.items():
        category_labels.append(str(label))  # Ensure label is string
        if callable(value):
    ...
    print(f"Category data debug: {category_data}")  # Debug print
